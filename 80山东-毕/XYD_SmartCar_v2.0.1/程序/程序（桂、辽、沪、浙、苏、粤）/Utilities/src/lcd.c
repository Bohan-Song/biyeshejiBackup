...
void Lcd_Gpio_Init(void)
{
	...
	GPIO_WriteBit(LCD_RD_PORT, LCD_RD_PIN,Bit_SET);
	GPIO_WriteBit(LCD_WR_PORT, LCD_WR_PIN,Bit_SET);
	GPIO_WriteBit(LCD_RS_PORT, LCD_RS_PIN,Bit_SET);
	GPIO_WriteBit(LCD_CS_PORT, LCD_CS_PIN,Bit_RESET);//片选，拉低有效
}
void LCD_Writ_Bus(unsigned short bus_data)//16位
{
	LCD_DATA_PORT->ODR = bus_data;
	GPIO_WriteBit(LCD_WR_PORT, LCD_WR_PIN,Bit_RESET);
	GPIO_WriteBit(LCD_WR_PORT, LCD_WR_PIN,Bit_SET);
}
...
void LCD_Init(void)
{
	GPIO_WriteBit(LCD_REST_PORT, LCD_REST_PIN,Bit_SET);//需要一定的延时，保证初始化进行
  delayms(10);	
	GPIO_WriteBit(LCD_REST_PORT, LCD_REST_PIN,Bit_RESET);
	delayms(10);
	GPIO_WriteBit(LCD_REST_PORT, LCD_REST_PIN,Bit_SET);
	delayms(10);
	LCD_Write_COM(0xCF);  
	LCD_Write_DATA(0x00); 
	LCD_Write_DATA(0xC1); 
...
//     LCD_CS_CLR;     
    ...
    //读取数据(读GRAM时,第一次为假读)   
//     LCD_RD_CLR; 
	GPIO_WriteBit(LCD_RD_PORT, LCD_RD_PIN,Bit_RESET);
    delay_us(100);//延时1us                    
//     LCD_RD_SET;
	GPIO_WriteBit(LCD_RD_PORT, LCD_RD_PIN,Bit_SET);
    //dummy READ
