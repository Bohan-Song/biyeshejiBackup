...
void LED()//LED指示灯-提示
{
	GPIO_WriteBit(LED1_GPIO_PORT, LED1_GPIO_PIN,(BitAction)(LED_flag>>7));
	LED_flag=~LED_flag;
}
void Data_LCD_Display()//常规显示
{
	...
	GPIO_WriteBit(FIFO_WRST_PORT, FIFO_WRST_PIN, Bit_RESET);
...
//第2个字符：
	...
	// 初始车位数为0  如果匹配成功我们以字符串“0000”代替 通过比较当前状态 和上时刻状态来对比 车子是进入还是出去
	
	//增加判断车位信息 通过flag来判断 如果flag=0 推荐第一个车位 以此类推
	
	RecommendChewei(flag);
	current = state(flag);
	chewei =  stopC();
	
	if(strcmp(current,chewei)==0)
	{	
		openLan();//打开栏杆
		//保存车牌信息 这是进入车辆
	for(int i =0;i<7;i++)
	{
		table_card[flag][i] = table_cardMeasure[i];
	}
	flag++;
	closeLan();//关闭
	}
	...
	{
		openLan();//打开栏杆
		check = check_C(current,chewei);
		//删除出去的车牌
	for(int i =0;i<7;i++)
	{
		table_card[check-1][i] = table_cardMeasure[i];
	}
	closeLan();//关闭栏杆
	}
				LCD_Fill(0x00);//黑屏
...
void Data_LCD_ColorChange()//摄像头扫描
{
	...
	if(flag_MaxMinCompare==1)//跳变点筛选成功  左右边界获取成功 并且合理
	{
		...
		{
			LCD_Fill(BLACK);//黑屏，显示Measure Faill
...
{ 		    		  			    
	if(TIM3->SR&0X0001)//溢出中断
	{
		...
		if(tim3_num==10000)
		{
			if(table_card[0][0]!=0&&HC1==1)//第1组计时
			{
				if(table_card[0][7]<=20)
				{
					table_card[0][7]++;
				}
				else
			...
			if(table_card[1][0]!=0&&HC2==1)//第2组计时
			{
				if(table_card[1][7]<=20)
				{
					table_card[1][7]++;
				}
				else
			...
			if(table_card[2][0]!=0&&HC3==1)//第3组计时
			{
				if(table_card[2][7]<=20)
				{
					table_card[2][7]++;
				}
				else
			...
			if(table_card[3][0]!=0&&HC4==1)//第4组计时
			{
				if(table_card[3][7]<=20)
				{
					table_card[3][7]++;
				}
				else
...
{  
	unsigned int num=0;
	float weiyi=0;
	Stm32_Clock_Init(14);//初始化时钟
	Led_init();			//初始化 LED	
	Lcd_Gpio_Init();
	LCD_Init();	
	Key_init();	//初始化 KEY1 PA8
	OV7670_Gpio_Init();//OV7670引脚初始化，放在串口初始化前面
	GPIO_WriteBit(FIFO_OE_PORT, FIFO_OE_PIN, Bit_RESET);
	USART1_init();//初始化串口	
	TIM3_Configuration();//10Khz的计数频率，计数到5000为500ms 
	TIM4_PWM_Init(200-1,7200-1);     //时钟为72MHZ   72000000/200/7200=50    分频次数  计数个数  sg90舵机需要的是20msPWM波，50hz，设置占空比来0.5ms-0°  2.5ms-180°	
	closeLan();    //初始化栅栏关闭
	LCD_Fill(BLUE);		
	printf("Welecom\r\n");
	while(!Sensor_init());
